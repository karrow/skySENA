...
long sky::LineaBlanca() {
  float posicion = LinePosDigital();
  posicion = map(posicion, 1.0, 8.0, -255, 255);
  return posicion;
}

long sky::LineaNegra() {
  float posicion = LinePosDigital();
  posicion = posicion*100/7 - 100/7;
...
void sky::PID_Sky(float kp, float kd, float ki) {
...
long sky::PID(int POS, int setpoint, int lim) {
